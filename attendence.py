...
face_locations = []
face_encodings = []
attendance_record = {}  # Dictionary to store attendance
while True:
    ...
    for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
        ...
        # Record the attendance of the recognized person
        if name != "Unknown":
            if name not in attendance_record:
                attendance_record[name] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
